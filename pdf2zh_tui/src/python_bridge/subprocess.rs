...
impl PythonProcess {
    ...
    /// Tries the entry point command first, then falls back to python -m.
    pub async fn spawn(python_path: &str) -> Result<Self> {
        // 输出 PDF 默认保存到下载目录
        ...
        let mut child = match child_result {
            ...
            Err(_) => {
                ...
                Command::new(python_path)
                    ...
                    .with_context(|| {
                        format!(
                            "Failed to spawn pdf2zh_next. Tried 'pdf2zh_next' and '{python_path} -m pdf2zh_next'"
                        )
                    })?
            }
        };
        ...
        // Channel for receiving events from stdout
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                match serde_json::from_str::<PythonEvent>(&line) {
                    ...
                    Err(_) => {
                        // Ignore unparseable lines (Python startup noise, etc.)
                    }
                }
            }
        });
